...
/**
 * The runs of `run`'s task, as a source named `alias`. MySQL refuses a subquery that
 * reads the table its statement updates (error 1093) unless the read goes through a
 * derived table, and these predicates guard updates of `runs` as well as reads. The
 * correlation sits inside the derived table, so MySQL materializes one task's runs
 * through the `(task_id, attempt)` index and never the whole table.
 */
export const runsOfTask = (alias: string, run: string): string =>
  `(SELECT ${alias}_run.* FROM runs ${alias}_run
    WHERE ${alias}_run.task_id = ${run}.task_id) ${alias}`
...
export const registeredWait = (
  ...
  const witness = `w.run_id = ${run}.run_id
      ...
      AND w.timeout_at_ms <=> ${run}.available_at_ms
...
/**
 * MySQL's BIGINT columns hold nothing but integers: a non-numeric value is refused
 * under the strict mode every session sets, and a fractional one is rounded on the
 * way in. A non-null check therefore is the native exact-integer proof; the
 * semantic bounds remain explicit below.
 */
...
export const epochAdditionFits = (base: string, ...deltas: readonly string[]): string => {
  ...
  const totalDelta = deltas.map((delta) => `CAST((${delta}) AS SIGNED)`).join(' + ')
...
export const storedHighestOwnedOrdinal = (run: string): string => {
  ...
  return `NOT EXISTS (
    SELECT 1 FROM ${runsOfTask('higher', run)}
    WHERE (NOT ${storedIntegerWithin(attempt, 'higher')}
...
/**
 * Prove a stored value is JSON before any JSON function parses it. `JSON_VALID` answers
 * false for text MySQL cannot hold as JSON, such as a number past a double or a lone
 * surrogate escape, where `JSON_EXTRACT` would raise and abort the statement. Every
 * CASE below tests it first.
 */
 ...
 *
 * MySQL keeps the last of two members with one key, as `JSON.parse` does, and has no
 * way to ask whether a key occurred twice. So this guard and the decoder read the same
 * member, and a repeated key is not refused here as it is on the other dialects.
 */
export const durableTaskRetryAdmissible = (task: string): string => {
  const retry = `${task}.retry_strategy`
  const member = (key: string): string => `JSON_EXTRACT(${retry}, '$.${key}')`
  const kind = `JSON_UNQUOTE(${member('kind')})`
  const duration = (key: string): string => {
    const value = `CAST(${member(key)} AS DOUBLE)`
    return `(CASE
      WHEN JSON_TYPE(${member(key)}) NOT IN ${JSON_NUMBER_TYPES} THEN FALSE
  ...
  const factorAdmissible = `(CASE
      WHEN JSON_TYPE(${member('factor')}) NOT IN ${JSON_NUMBER_TYPES} THEN FALSE
      ELSE CAST(${member('factor')} AS DOUBLE) BETWEEN 0 AND 1.7976931348623157e308
    END)`
  return `(
    CASE
      WHEN NOT ${jsonInputValid(retry)} THEN 0
      WHEN JSON_TYPE(CAST(${retry} AS JSON)) <> 'OBJECT' THEN 0
      WHEN JSON_TYPE(${member('kind')}) <> 'STRING' THEN 0
...
export const durableTaskHeadersAdmissible = (task: string): string => {
  ...
  return `(
    CASE
      ...
      WHEN NOT ${jsonInputValid(headers)} THEN 0
      WHEN JSON_TYPE(CAST(${headers} AS JSON)) <> 'OBJECT' THEN 0
      WHEN EXISTS (
        SELECT 1 FROM JSON_TABLE(${headers}, '$.*' COLUMNS (member JSON PATH '$')) h
...
export const soleLiveRun = (run: string): string =>
  `NOT EXISTS (SELECT 1 FROM ${runsOfTask('sibling', run)}
               WHERE sibling.state IN ${LIVE}
...
const rollbackRan = (marker: string, prefix: string): string =>
  `EXISTS (SELECT 1 FROM checkpoints sr
           WHERE sr.task_id = ${marker}.task_id
             AND sr.checkpoint_name = CONCAT('${SAGA_ROLLBACK_PREFIX}',
