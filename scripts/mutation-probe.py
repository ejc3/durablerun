...
MUTATION_SPECS = [
    ...
    (
        ...
        # statement tree, and each has one test in fenced-batch-tree-verdicts.test.ts. The
        # text-path entries above that hold the same rules are retired with the text path.
    ...
    (
        "tree-tie-exists-inner-column",
        "packages/core/src/sql-tree.ts",
        "    return (isInner(left) && isOuter(right)) || (isOuter(left) && isInner(right))",
        "    return isOuter(right) || isOuter(left)",
        "a comparison that never reads the fenced source counts as a tie",
    ),
    (
        "tree-tie-exists-outer-column",
        "packages/core/src/sql-tree.ts",
        "    return (isInner(left) && isOuter(right)) || (isOuter(left) && isInner(right))",
        "    return isInner(left) || isInner(right)",
        "a comparison that never reads the outer row counts as a tie",
    ),
    (
...
VERDICTS = {
    ...
    "tree-provenance-assigned-once": ExpectedVerdict(
        "construction",
        "packages/core/test/fenced-batch-tree-verdicts.test.ts",
        "the tree path stamping refuses a provenance column assigned twice",
    ...
    "tree-tie-exists-inner-column": ExpectedVerdict(
        "construction",
        "packages/core/test/fenced-batch-tree-verdicts.test.ts",
        "the tree path the tie of a subquery gate to the fenced source ties EXISTS by a column of the fenced source on one side",
        "mutation-verdict:construction:tree-tie-exists-inner-column",
    ),
    "tree-tie-exists-outer-column": ExpectedVerdict(
        "construction",
        "packages/core/test/fenced-batch-tree-verdicts.test.ts",
        "the tree path the tie of a subquery gate to the fenced source ties EXISTS by a column of the outer row on the other side",
        "mutation-verdict:construction:tree-tie-exists-outer-column",
    ),
    ...
    "tree-raw-fragment-order": ExpectedVerdict(
        "construction",
        "packages/core/test/fenced-batch-tree-verdicts.test.ts",
        "the tree path the statement, its fragments, and its binds refuses a raw node rawSql did not mint",
