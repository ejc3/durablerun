...
MUTATION_SPECS = [
    ...
    (
        ...
        "    } else if (false && PrimitiveValueListNode.is(node)) {",
    ...
    (
        ...
        "    } else if (ValueListNode.is(node)) {\n"
        "      problem = stateListProblem(\n"
        "        node.values.map((value) => (ValueNode.is(value) ? undefined : undefined)),",
    ...
    (
        ...
        "a list that names no state is refused as a set of states",
    ),
    (
    ...
    (
        "tree-state-list-skips-insert-rows",
        "packages/core/src/sql-tree.ts",
        "    if (ValuesNode.is(node)) {\n"
        "      rows.push(...node.values)",
        "    if (false && ValuesNode.is(node)) {\n"
        "      rows.push(...node.values)",
        "a row of inserted values that names a state is refused as a list of states",
    ),
    (
    ...
    (
        ...
        "        problem = stateListProblem(readFragment(list).literals) ?? null",
        "a defined list later in a fragment clears the problem of a list before it",
    ),
    (
    ...
    (
        "tree-deadline-operator-less",
        "packages/core/src/sql-tree.ts",
        "const ORDERING_OPERATORS = ['<', '<=', '>', '>=']",
        "const ORDERING_OPERATORS = ['<=', '>', '>=']",
        "a deadline compared with < passes when it is built from nodes",
    ),
    (
        "tree-deadline-operator-less-or-equal",
        "packages/core/src/sql-tree.ts",
        "const ORDERING_OPERATORS = ['<', '<=', '>', '>=']",
        "const ORDERING_OPERATORS = ['<', '>', '>=']",
        "a deadline compared with <= passes when it is built from nodes",
    ...
    (
        "tree-deadline-operator-greater",
        "packages/core/src/sql-tree.ts",
        "const ORDERING_OPERATORS = ['<', '<=', '>', '>=']",
        "const ORDERING_OPERATORS = ['<', '<=', '>=']",
        "a deadline compared with > passes when it is built from nodes",
    ...
    (
        "tree-deadline-operator-greater-or-equal",
        "packages/core/src/sql-tree.ts",
        "const ORDERING_OPERATORS = ['<', '<=', '>', '>=']",
        "const ORDERING_OPERATORS = ['<', '<=', '>']",
        "a deadline compared with >= passes when it is built from nodes",
    ...
    (
        ...
        "      const compared = [node.leftOperand].some((operand) =>",
    ...
    (
        ...
        "        referencedColumn(operand) === 'cancel_at_ms',",
        "a deadline comparison passes when arithmetic surrounds the column",
    ),
    (
        "tree-deadline-orders",
        "packages/core/src/sql-tree.ts",
        "      ORDERING_OPERATORS.includes(operatorName(node.operator) ?? '')",
        "      (operatorName(node.operator) ?? '') !== undefined",
        "a test of the deadline that orders nothing is refused as a comparison",
    ),
    (
        ...
        "      if (true || compared) {\n"
        "        problem =",
        "an ordering of any column is refused as a deadline comparison",
    ),
    (
...
MUTATION_SPECS.extend(
    (
        ...
        (
            ...
            "    // MUTATION: activate remembers nothing\n",
        ...
        (
            ...
            "  if (typeof parsed === 'undefined') { // MUTATION\n",
            "a completion event that holds JSON null crashes the decoder",
        ),
        (
            "task-outcome-refuses-live-state",
            "packages/core/src/child-tasks.ts",
            "  if (!isTerminalState(state)) {\n",
            "  if (state === null) { // MUTATION\n",
        ...
        (
            ...
            "    if (this.#tasks.size < 0) { // MUTATION\n",
            "a long-lived store remembers the task of every run it ever activated",
        ),
        (
        ...
        (
            ...
            "           'durablerun:claim', 'events'::regclass::oid::text, $1::text, $2::text\n",
            "a completion event's advisory key carries the claim lock's tag",
        ),
        (
        ...
        (
            ...
            "a parent that awaits a child in another queue retries a refusal that can never change",
        ),
        (
        ...
        (
            "wake-sources-find-the-woken-runs",
            "packages/store-libsql/src/store.ts",
            "      where: `f.queue = ? AND f.wake_event = ? AND f.state = 'pending'`,\n      whereArgs: [queue, eventName],\n      rows: 'source-keys',\n",
            "      where: `f.queue = ? AND ? IS NOT NULL AND f.state = 'pending'`,\n      whereArgs: [queue, eventName],\n      rows: 'source-keys',\n",
            "every task ending walks every pending run of its queue to find the runs it woke",
        ),
        (
        ...
        (
            "sdk-child-outcome-error-is-permanent",
            "packages/sdk/src/context.ts",
            "      if (error instanceof ChildAwaitRefusedError || error instanceof RangeError) {\n",
            "      if (error instanceof ChildAwaitRefusedError) {\n",
            "a child's recorded outcome that cannot be read is retried until the budget is gone",
        ),
        (
            "postgres-port-event-lock-is-the-row-older-builds-take",
            "packages/store-postgres/src/executor.ts",
            "  if (lock.kind === 'event' && !lock.eventName.startsWith(RESERVED_EVENT_PREFIX)) {\n",
            "  if (lock.kind === 'event' && lock.eventName.startsWith(RESERVED_EVENT_PREFIX)) {\n",
            "an older build's emit and this build's await of one event do not exclude each other, and the waiter sleeps for ever",
        ),
        (
            "postgres-port-event-lock-inserts-its-row",
            "packages/store-postgres/src/executor.ts",
            "    await client.query(\n      `INSERT INTO event_locks (queue, event_name)\n       VALUES ($1, $2)\n       ON CONFLICT (queue, event_name) DO NOTHING`,\n      args,\n    )\n",
            "",
            "an event nobody has locked before has no row to lock, so its first emit and await take no lock at all",
        ),
        (
            "batch-never-skips-a-tail-that-always-answers",
            "packages/core/src/fenced-batch.ts",
            "      const alwaysAnswers = !mayReturnNoRow(tree)\n",
            "      const alwaysAnswers = false // MUTATION\n",
            "a counting tail is skipped on PostgreSQL and answers with no row there and with 0 on libSQL",
        ),
        (
            "batch-states-only-a-gate-it-found",
            "packages/core/src/fenced-batch.ts",
            "      if (gateIndex >= 0 && !alwaysAnswers) gatedBy = gateIndex\n",
            "      if (!alwaysAnswers) gatedBy = gateIndex\n",
            "a statement with no gate reaches the executor gated on a statement that does not exist",
        ),
        (
            "only-a-select-can-always-answer",
            "packages/core/src/sql-tree.ts",
            "  if (!SelectQueryNode.is(query)) return true\n",
            "  if (!SelectQueryNode.is(query)) return false\n",
            "no follow-on is ever skipped, and PostgreSQL pays a round trip for every statement a lost gate made a no-op",
        ),
        (
        ...
        (
            "child-spawn-refusal-is-the-runs",
            "packages/store-libsql/src/store.ts",
            "      if (childOf !== undefined) throw await this.refusal('spawn', childOf.runId)\n",
            "",
            "a child spawn under a lost claim is reported as an engine fault and not as the lost lease it is",
        ),
        (
            "child-spawn-key-is-unambiguous",
            "packages/core/src/child-tasks.ts",
            ...
            "two pairs of parent and call site spell one key, and the second spawn adopts the first one's task",
        ),
    )
)
...
VERDICTS = {
    ...
    "tree-state-list-names-a-state": ExpectedVerdict(
        "construction",
        "packages/core/test/sql-tree-verdicts.test.ts",
        "a second definition of eligibility a list of states accepts a list that names no state",
    ...
    "tree-state-list-skips-insert-rows": ExpectedVerdict(
        "construction",
        "packages/core/test/sql-tree-verdicts.test.ts",
        "a second definition of eligibility a list of states reads a row of inserted values as a row, and as no list of states",
        "mutation-verdict:construction:tree-state-list-skips-insert-rows",
    ),
    "tree-state-list-in-a-fragment": ExpectedVerdict(
        "construction",
        "packages/core/test/sql-tree-verdicts.test.ts",
        "a second definition of eligibility a list of states refuses such a list in the text of a fragment, whatever stands before it",
        "mutation-verdict:construction:tree-state-list-in-a-fragment",
    ),
    "tree-state-list-every-list-of-a-fragment": ExpectedVerdict(
        "construction",
        "packages/core/test/sql-tree-verdicts.test.ts",
        "a second definition of eligibility a list of states reads every list of a fragment, and a defined one after it clears nothing",
    ...
    "tree-deadline-operator-less": ExpectedVerdict(
        "construction",
        "packages/core/test/sql-tree-verdicts.test.ts",
        "a second definition of eligibility a comparison of the cancellation deadline refuses cancel_at_ms < a value, built from nodes",
        "mutation-verdict:construction:tree-deadline-operator-less",
    ),
    "tree-deadline-operator-less-or-equal": ExpectedVerdict(
        "construction",
        "packages/core/test/sql-tree-verdicts.test.ts",
        "a second definition of eligibility a comparison of the cancellation deadline refuses cancel_at_ms <= a value, built from nodes",
        "mutation-verdict:construction:tree-deadline-operator-less-or-equal",
    ),
    "tree-deadline-operator-greater": ExpectedVerdict(
        "construction",
        "packages/core/test/sql-tree-verdicts.test.ts",
        "a second definition of eligibility a comparison of the cancellation deadline refuses cancel_at_ms > a value, built from nodes",
        "mutation-verdict:construction:tree-deadline-operator-greater",
    ),
    "tree-deadline-operator-greater-or-equal": ExpectedVerdict(
        "construction",
        "packages/core/test/sql-tree-verdicts.test.ts",
        "a second definition of eligibility a comparison of the cancellation deadline refuses cancel_at_ms >= a value, built from nodes",
    ...
    "tree-deadline-either-side": ExpectedVerdict(
        "construction",
        "packages/core/test/sql-tree-verdicts.test.ts",
        "a second definition of eligibility a comparison of the cancellation deadline refuses it with the column on the right",
        "mutation-verdict:construction:tree-deadline-either-side",
    ),
    "tree-deadline-under-arithmetic": ExpectedVerdict(
        "construction",
        "packages/core/test/sql-tree-verdicts.test.ts",
        "a second definition of eligibility a comparison of the cancellation deadline refuses it with arithmetic around the column",
        "mutation-verdict:construction:tree-deadline-under-arithmetic",
    ),
    "tree-deadline-orders": ExpectedVerdict(
        "construction",
        "packages/core/test/sql-tree-verdicts.test.ts",
        "a second definition of eligibility a comparison of the cancellation deadline accepts a test of the column that orders nothing",
        "mutation-verdict:construction:tree-deadline-orders",
    ),
    "tree-deadline-names-the-column": ExpectedVerdict(
        "construction",
        "packages/core/test/sql-tree-verdicts.test.ts",
        "a second definition of eligibility a comparison of the cancellation deadline accepts an ordering of another column",
...
for _verdict, _names in (
    ...
    (
        ExpectedVerdict(
            "behavior",
            "packages/store-postgres/test/executor.test.ts",
            "PgExecutor transactions locks a task's completion event with a scoped advisory lock, and leaves no row",
    ...
    (
        ExpectedVerdict(
            "behavior",
            "packages/store-libsql/test/query-plans.test.ts",
            "a terminal batch's wake, which every task ending pays finds the runs it woke without walking the pending runs of the queue",
            "mutation-verdict:behavior:wake-sources-find-the-woken-runs",
        ),
        (
            "wake-sources-find-the-woken-runs",
        ),
    ...
    (
        ExpectedVerdict(
            "behavior",
            "packages/conformance/test/libsql.test.ts",
            "child task conformance [libsql] registers on a child that is revived before the read that says why",
    ...
    (
        ExpectedVerdict(
            "behavior",
            "packages/sdk/test/child-tasks.test.ts",
            "child tasks through the SDK fails the parent for good when the child's recorded outcome cannot be read",
            "mutation-verdict:behavior:sdk-child-outcome-error-is-permanent",
        ),
        (
            "sdk-child-outcome-error-is-permanent",
        ),
    ...
    (
        ExpectedVerdict(
            "behavior",
            "packages/conformance/test/postgres-terminal-lock.test.ts",
            "an older build's emit and this build's await of one event exclude each other, and the other way round",
    ...
    (
        ExpectedVerdict(
            "behavior",
            "packages/store-postgres/test/executor.test.ts",
            "PgExecutor transactions acquires the event row lock before protocol SQL without adding a result",
    ...
    (
        ExpectedVerdict(
            "behavior",
            "packages/core/test/fenced-batch-tree.test.ts",
            "FencedBatch tree statements never skips a gated tail that answers with a row whatever it matched",
    ...
    (
        ExpectedVerdict(
            "behavior",
            "packages/conformance/test/libsql.test.ts",
            "scheduler conformance [libsql] spawn creates a child only under its parent's live claim, and still finds one that exists",
    ...
    (
        ExpectedVerdict(
            "behavior",
            "packages/core/test/child-tasks.test.ts",
            "a child's spawn key is a different key for every pair of parent and call site",
...
def run_suite_process(
    ...
    *,
    ...
    """Run one verifier suite and own its whole process group.

    The wall-time limit bounds the leader. Once the leader exits, the group gets up
    to drain_grace_seconds to empty before cleanup terminates what remains, so a
    suite can take the wall-time limit plus the drain grace plus the verifier
    termination graces.
    """
...
# Each suite self-test fault and the argparse destination of the child it belongs to.
SUITE_SELF_TEST_FAULT_CHILDREN = {
    "immediate-magic-error": "suite_timeout_self_test_child",
    "reject-draining-group": "suite_drain_self_test_child",
    "wall-time-instead-of-descendant": "suite_linger_self_test_child",
}
...
stream.write(
    json.dumps({"label": sys.argv[1], "leader": int(sys.argv[3]), "descendant": os.getpid()})
    + "\\n"
)
stream.flush()
...
DrainWait = Callable[[list[int], float], list[int]]


@contextlib.contextmanager
def drain_wait_replaced(
    replacement: Callable[[DrainWait, list[int], float], list[int]],
) -> Iterator[None]:
    """Make both runners wait for group drains through replacement.

    The replacement receives the wait that was in place when the swap began.
    """
    original_wait = wait_for_process_groups

    def replaced(groups: list[int], grace_seconds: float) -> list[int]:
        return replacement(original_wait, groups, grace_seconds)

    globals()["wait_for_process_groups"] = replaced
    try:
        yield
    finally:
...
def barrier_released_on_drain(
    ...
    against the runner's polling. With after_check, the barrier appears only once
...
def suite_self_test_verifier_run(
    ...
    *,
    ...
    the problems found in the verifier's records and processes. With leader_exits
    False the leader keeps running after its descendant records itself.
    """
    problems: list[str] = []
...
def suite_linger_self_test_child(state_path: Path, fault: str | None) -> int:
    ...
    Under wall-time-instead-of-descendant the leader keeps running after its
...
def suite_drain_self_test_child(state_path: Path, fault: str | None) -> int:
    """A verifier group that drains just after its leader exits must be accepted.
...
TREE_RULE_SPELLING_BLOCKS: tuple[tuple[str, str, str], ...] = (
    ...
    ("packages/core/src/sql-tree.ts", "const ORDERING_OPERATORS = ", "\n"),
)
...
TREE_CONDITIONS_WITHOUT_A_MUTATION: dict[str, dict[str, str]] = {
    ...
    "packages/core/src/sql-tree.ts": {
        ...
        "BinaryOperationNode.is(node) &&": (
            "fails closed: every mutant an automatic sweep made of this line fails ordinary tests, 307 at the fewest"
        ),
        ...
        "if (!values.some((value) => typeof value === 'string' && STATE_NAMES.includes(value))) return null": (
            "changes nothing a statement can show: with the string test gone the state names are asked about a value that is no string, and they hold none"
        ),
...
def reap_leftover_process(launch: ProcessLaunch) -> str | None:
    ...
    Returns the failure to record, naming the launch, or None when nothing was left.
    """
...
def orchestration_self_test(fault: str | None = None) -> int:
    ...
    with (
        ...
        if fault in (None, "reject-draining-group"):
            ...
            # starts draining. Under the fault the runner checks with no drain and the
        ...
        if fault in (None, "accept-lingering-group", "drop-live-groups-while-polling"):
            ...
            broken_waits = {
                ...
                "drop-live-groups-while-polling": drop_live_groups_while_polling,
            }
            try:
                with (
                    drain_wait_replaced(broken_waits[fault])
                    if fault
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        choices=tuple(SUITE_SELF_TEST_FAULT_CHILDREN),
        help=argparse.SUPPRESS,
    )
    ...
    if any(self_test_modes):
        ...
        if suite_process_self_test:
            ...
            if args.suite_self_test_fault is not None:
                fault_child = SUITE_SELF_TEST_FAULT_CHILDREN[args.suite_self_test_fault]
                if not getattr(args, fault_child):
                    ap.error(
                        f"--suite-self-test-fault {args.suite_self_test_fault} "
                        f"requires --{fault_child.replace('_', '-')}"
                    )
            ...
            if args.suite_linger_self_test_child:
                return suite_linger_self_test_child(
                    args.suite_self_test_state,
                    args.suite_self_test_fault,
                )
