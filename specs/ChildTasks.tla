...
\*    event.  Retention.tla models the purge that does, beside these actions: the
\*    event goes only with its task's whole unit, when no holder needs it, and an
\*    await after that is refused and registers nothing.
