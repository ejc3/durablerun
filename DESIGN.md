...
1. **Fenced batches, keyed on the post-state.** Batches are atomic but
   ...
   **Statements as trees.** A batch statement may be a Kysely operation tree.
   ...
   - A store's reads outside a transition are batches of reads. `readTree`
     ...
     placeholder no partial index can match. The bound value is found wherever
     it stands below the right side of the test, as the column is found below
     the left: alone, in parentheses, in a list under IN or NOT IN, where the
     builder binds every plain value, or under a cast, a call, a CASE or a
     value fragment that carries a bind. A subquery on the right is its own
     statement and is not read into. A list of inline literals is admitted,
     because that is the form a partial index matches. A comparison written
     whole inside a store fragment is not read, because a tree carries a
     fragment as text. MySQL builds its own `next-wake`,
