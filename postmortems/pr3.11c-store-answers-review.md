...
| 9 | A throwing accessor on a wake field escaped the decoder (PR review) | The pass rejects with no field named | The store answer surface | Its getter case threw only from a top-level field | The decoder reads and decodes each field, nested accessors included, inside one try (rung 1). The surface sends a throwing accessor for each wake field (rung 3) |
...
Before review, the store answer surface caught the original defect this PR exists for, a handler running on an answer missing required fields. The nine review-caught defects all sat outside the inputs the surface generated.
...
- **An unreadable field recurred one level down, within this round.** Finding 7's fix guarded the top-level read, and finding 9 is the same class in the wake's own fields. The surface generated the getter case only where the instance was found, the same instance-derived axis this document names as the root cause.
...
Six. Findings 4 to 9 were introduced by `d8299c1`, the fix for round 1. It multiplied the lease by 1000 and demanded an exact integer. It reused `decodeBoundedInteger`, which accepts bigint, and checked fields one at a time. It read fields directly, and required `timedOut === undefined` for a delivered wake. Round 2 covered `d8299c1` as new code, which is how findings 4 to 8 were found. Finding 9 survived `b66f919`, whose getter fix covered only top-level reads, and the PR review covered `b66f919` as new code.
...
- Finder of finding 9: the Greptile PR review over `ae56072`, quoted verdict: "If a malformed activation answer defines a throwing accessor for `wake.event`, `wake.step`, `wake.timedOut`, or `wake.payloadJson`, these reads occur outside the `try` that protects the top-level field access".
