...
could exit before printing its promised infrastructure diagnosis. The final
verdict remains an infrastructure and release-safety failure, not a protocol
counterexample.
...
false. The same commit also promised an `unreadable` infrastructure diagnosis
that `set -euo pipefail` could prevent from running.
...
Self-catch rate: **1 of 3, or 33%** (previous verification-infrastructure round:
**2 of 2, or 100%**).

The rate fell from 100% to 33%. Our nightly stopped the original line, but the
author's machinery accepted both defects introduced by its repair. A human
reviewer had to inspect the embedded archive and execute the untested shell
failure path. Detection worked for dependency drift; it did not work for the
new redistribution and diagnostic surfaces.
...
The observed verdict was five passing tests. This is an intentional boundary:
...
**Two: findings 2 and 3.** Both were introduced by commit `e97ae81`, the repair
for finding 1. The bundled binary created a licensing surface that the repair
described incorrectly, and its new error handler contained the unreachable
fallback. Both received failing regression tests in `3daac08` before green fix
`bdab7e7`; the repaired code was then rerun through the
five-case focused suite rather than merely assumed safe because the original
availability test remained green.
...
- Follow-up fixes: commit `bdab7e7` totalizes the checksum
  diagnostic and records the fat JAR's embedded notices, source links, pinned
  source tree, and build recipe. The current focused verdict is five of five
  tests green.
...
- The six isolated full-nightly targets still require a remote run on the fixed
  head. Until those complete, the artifact defect is fixed and the CI-sized
  model is green, but the repository is not described as fully healthy.
...
though it created two additional surfaces: redistribution obligations and a
new preflight diagnostic. Project-level licensing was substituted for an
inventory of the actual archive, and one readable-corruption example was
substituted for the error handler's failure domain under `set -euo pipefail`.
The common cause across the review findings was verifying the old outcome
without enumerating the surfaces created by the fix itself.

## Mechanisms

Built across commits `e97ae81` and `bdab7e7`:
...
- `scripts/tla.sh` validates the repository copy's fixed SHA-256 on every
  ...
  starts (rung 3).
- Five focused cases use a fake curl that records and fails any former download
  path, prove the repository artifact is selected, exercise missing, corrupt,
  and digest-command-failure paths, and require the current license/source
  record (rung 3).
- `tools/tla/README.md` identifies the JAR as a multi-license distribution,
  inventories its embedded EPL-2.0/GPL-2.0-with-Classpath-Exception,
  Apache-2.0, and BSD-3-Clause notices, and links the corresponding Jakarta
  Mail sources plus the pinned upstream tree and build recipe (rung 3; the
  hardcoded-manifest boundary is explicit).

Deferred (recorded in BUILD.md):

- The six isolated full-volume nightly targets must turn green on the remote
  fixed head. The CI-sized run already checks the model and all five temporal
  branches; the remote matrix supplies the larger state-space and runner-
  isolation evidence that a constrained all-local concurrent run cannot.
...
hardcoded license manifest can also remain green when the archive's dependency
inventory changes, so every checker update still requires archive and source
review. A shell prerequisite outside the three enrolled artifact cases can
...
state-space resource limits. Until the six remote full targets pass, the repair
has CI-scale model evidence but not a restored full-nightly receipt.
