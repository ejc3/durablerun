...
the same six conformance surfaces as libSQL. Adversarial review found five
...
checked-out clients left PostgreSQL `error` events without an owner. The
repairs close those concrete failures and add native concurrency, conversion,
and connection-lifecycle evidence; the honest verdict is that review, not the
branch's original machinery, found every one.
...
Self-catch rate: **0 of 5, or 0%** (previous round: **100% after merge, but
...
machinery working, but it is not one of these five escaped findings and does
...
| Opaque-value preservation regression | 2 | A new checkpoint path can call `serializeTaskHeaders(state)` while the current result and event payload preservation assertions remain green. The test protects the audited opaque paths, not a type-level distinction between inspectable objects and opaque JSON. |
...
The original failures were also executed, not inferred. Removing the
...
declared safe because the original header tests turned green.
...
- Fixes: commit `e68cf6d`. Focused green evidence before the final commit is 31 of 31 PostgreSQL package tests, 2 native same-token dialect cases, 2 header-ingress dialect cases, 8 of 8 PostgreSQL JSON hazard cases, 13 of 13 task-value cases, and 2 of 2 PostgreSQL pool-lifecycle cases. The final-head full verify, TLC, fuzz, and mutation results belong to the PR gate record and are not pre-claimed by this draft.
...
whether a preliminary PostgreSQL predicate guaranteed that its later casts
could not raise. It also modeled driver failures only as Promise rejections,
not as `error` events whose ownership changes when a client moves between idle
and checked-out states. SimWorld made batch scheduling deterministic by
treating the batch as one suspension point; that strength also made native
transaction races invisible. A new backend therefore arrived without a
generated fault surface for the adapter behaviors unique to its birth.
...
serialization was centralized, and Promise failures were caught; each
statement was true while the required property was false. The repairs move
current paths toward the properties and add real-backend counterexamples, but
...
- `jsonbInputValid` is the single conversion predicate used before current
  retry, header, and cancellation `jsonb` operations. Corrupt-storage cases
  cover candidate and same-token receipt paths, and the mutation probe replaces
  the authority with `TRUE` to prove attributable failure (rung 1 inventory
  plus rung 3 semantic attacks).
