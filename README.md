...
dogfood task records a durable repository-integrity journal: each checkpoint
reads a GitHub ref and records its commit SHA, tree SHA, and commit timestamp,
then sleeps without keeping a worker alive.
...
state, terminal failure reason, committed integrity checkpoints, retry
accounting, relaunches, and timestamps. The default 15 cycles contain 14
12-hour intervals, so the first-to-last checkpoint span is at least seven days.
...
set; `workflow_dispatch` can start, tick, or inspect it manually.
